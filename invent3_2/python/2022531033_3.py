...
for y, x in product(range(rows), range(cols)):
  if '\\' in data[y][x]:
    ...
    if former:
      ...
      while not(y + rows_flag == rows or '\\' in data[y+rows_flag][x] or 'x' in data[y+rows_flag][x]): 
    ...
    if latter:
      ...
      while not(x + cols_flag == cols or '\\' in data[y][x+cols_flag] or 'x' in data[y][x+cols_flag]):
...
for y, x in product(range(rows), range(cols)):
  for k in range(9):
    if (sol[y][x][k].x >= 0.99 and not ('\\' in data[y][x] or 'x' in data[y][x])):
      data[y][x] = str(k+1)
